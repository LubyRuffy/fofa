$(document).ready(function() {
    ...
    $('#tag_pillbox').on('added.fu.pillbox edited.fu.pillbox removed.fu.pillbox', function pillboxChanged() {
        tags = $('#tag_pillbox').pillbox('items');
        tags_string = '';
        for (var i = 0; i < tags.length; i++) {
            if (i>0)
                tags_string += ',';
            tags_string += tags[i].value;
        }
        $('#target_tag_list').val( tags_string );
    });
});
