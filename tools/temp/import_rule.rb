...
get_cms.each{|c|
  ...
  sql = "insert into rule(product, producturl, rule, created_at, user_id, published) values('#{Mysql2::Client.escape(name)}','#{Mysql2::Client.escape(url)}','#{Mysql2::Client.escape(rule)}','#{Mysql2::Client.escape(date)}',1,1)"
  puts sql
  @m.mysql.query(sql)
}